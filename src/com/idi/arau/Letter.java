...
public class Letter {
	...
	public Letter(char character, ViewGame view, Bitmap bmp, int maxSpeed) {
		...
		xSpeed = MIN_SPEED + rnd.nextInt(maxSpeed * 2) - maxSpeed;
		ySpeed = MIN_SPEED + rnd.nextInt(maxSpeed * 2) - maxSpeed;
	}

	public void update() {
		if (x >= view.getWidth() - letterPicture.getWidth() - xSpeed
				|| x + xSpeed <= 0) {
			xSpeed = -xSpeed;
		}
		x = x + xSpeed;

		if (y >= view.getHeight() - letterPicture.getHeight() - ySpeed
				|| y + ySpeed <= 0) {
			ySpeed = -ySpeed;
		}
		y = y + ySpeed;
	...
	public Boolean isTouch(float x2, float y2) {
		return x2 > x && x2 < x + letterPicture.getWidth() && y2 > y
				&& y2 < y + letterPicture.getHeight();
	}
